...
async def repo(client: Client, message: Message):
    await message.reply_photo(
        ...
        caption=f"""🍁𝐂𝐋𝐈𝐂𝐊🥰𝐁𝐄𝐋𝐎𝐖💝𝐁𝐔𝐓𝐓𝐎𝐍✨𝐓𝐎🙊𝐆𝐄𝐓🌱[𝐑𝐄𝐏𝐎](https://telegram.me/AB_KRISHNA_UFF)🍁""",
        reply_markup=InlineKeyboardMarkup(
            [
                [
                    InlineKeyboardButton(
                        "🌱ƨσʋяcɛ🌱", url=f"https://telegram.me/AB_KRISHNA_UFFC"
                    )
                ]
            ]
        ),
...
async def add_all(client, message):
    ...
    if len(command_parts) != 2:
        await message.reply(
            "**⚠️ ɪɴᴠᴀʟɪᴅ ᴄᴏᴍᴍᴀɴᴅ ғᴏʀᴍᴀᴛ. ᴘʟᴇᴀsᴇ ᴜsᴇ ʟɪᴋᴇ » `/gadd @AB_KRISHNA_UFF`**"
        )
        return
